...
GLProgressBar::GLProgressBar(float minValue, float maxValue, float width, float height, UISizeMode sizeMode)
...
void GLProgressBar::setCenter(float x, float y) {
    m_centerX = x;
    m_centerY = y;
    m_useCenter = true;
}

void GLProgressBar::setCenterX(float x) {
    m_centerX = x;
    m_useCenter = true;
}

void GLProgressBar::setCenterY(float y) {
    m_centerY = y;
    m_useCenter = true;
}
...
void GLProgressBar::setBarColors(const QVector4D& full, const QVector4D& mid, const QVector4D& low) {
    m_fullColor = full;
    m_midColor = mid;
    m_lowColor = low;
}
...
void GLProgressBar::render(QOpenGLFunctions_3_3_Core* gl, float screenWidth, float screenHeight, float x, float y) {
    if (!m_initialized) initialize(gl);

    // Calculate size in pixels
    float w = (m_sizeMode == UISizeMode::Fraction) ? m_width * screenWidth : m_width;
    float h = (m_sizeMode == UISizeMode::Fraction) ? m_height * screenHeight : m_height;
    ...
    if (m_useCenter) {
        float w = (m_sizeMode == UISizeMode::Fraction) ? m_width * screenWidth : m_width;
        float h = (m_sizeMode == UISizeMode::Fraction) ? m_height * screenHeight : m_height;
        xpos = ((m_sizeMode == UISizeMode::Fraction) ? m_centerX * screenWidth : m_centerX) - w / 2.0f;
        ypos = ((m_sizeMode == UISizeMode::Fraction) ? m_centerY * screenHeight : m_centerY) - h / 2.0f;
    } else {
    ...
    QVector2D uOffset_bg(
        (xpos * 2.0f / screenWidth) - 1.0f,
    ...
    QVector2D uScale_fg(w * percent * 2.0f / screenWidth, h * 2.0f / screenHeight);
