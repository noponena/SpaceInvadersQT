...
GLWeaponBar::GLWeaponBar(int slotCount, float slotSize, UISizeMode sizeMode)
    : m_slotCount(slotCount), m_slotSize(slotSize), m_sizeMode(sizeMode),
      m_posX(0.5f), m_posY(0.05f), m_posMode(UISizeMode::Fraction),
...
void GLWeaponBar::destroyGL(QOpenGLFunctions_3_3_Core *gl) {
  ...
  if (m_texVao)
    gl->glDeleteVertexArrays(1, &m_texVao);
  if (m_texVbo)
    gl->glDeleteBuffers(1, &m_texVbo);
}
...
void GLWeaponBar::startCooldown(int slotIdx, float cooldownSeconds,
                                float currentTimeSec) {
  if (slotIdx < 0 || slotIdx >= m_slotCount)
    return;
...
void GLWeaponBar::setBarBorderColor(const QVector4D &color) {
  m_barBorderColor = color;
}
void GLWeaponBar::setSlotBorderColor(const QVector4D &color) {
  m_slotBorderColor = color;
}
void GLWeaponBar::setSlotBackgroundColor(const QVector4D &color) {
  m_slotBackgroundColor = color;
}
...
void GLWeaponBar::setSlotImage(int index, const QString &imagePath) {
  ...
  if (!imagePath.isEmpty()) {
    const auto &texInfo =
        Graphics::TextureRegistry::instance().getOrCreateTexture(imagePath);
...
void GLWeaponBar::initialize(QOpenGLFunctions_3_3_Core *gl) {
  ...
  if (!m_textureProgram) {
    ...
    gl->glBufferData(GL_ARRAY_BUFFER, sizeof(quadVerts), quadVerts,
                     GL_STATIC_DRAW);
  ...
  if (!m_cooldownPieProgram) {
    ...
    m_cooldownPieProgram->addShaderFromSourceCode(
        QOpenGLShader::Vertex, texturedVertSrc); // same as icon shader
    m_cooldownPieProgram->addShaderFromSourceCode(QOpenGLShader::Fragment,
                                                  cooldownTexturedFragSrc);
...
void GLWeaponBar::render(QOpenGLFunctions_3_3_Core *gl, float screenWidth,
                         float screenHeight, float currentGameTime) {
  if (!m_initialized)
    initialize(gl);

  // Calculate slot size in pixels
  float slotPx = (m_sizeMode == UISizeMode::Fraction) ? m_slotSize * screenWidth
                                                      : m_slotSize;
  ...
  float y =
      (m_posMode == UISizeMode::Fraction) ? m_posY * screenHeight : m_posY;
  ...
  QVector2D uOffset_bar(((x - outerBorder) * 2.0f / screenWidth) - 1.0f,
                        1.0f - ((y - outerBorder) * 2.0f / screenHeight) -
                            uScale_bar.y());
  ...
  for (int i = 0; i < m_slotCount; ++i) {
    ...
    QVector2D uOffset_slotBorder(
        ((slotX - sBorder) * 2.0f / screenWidth) - 1.0f,
        1.0f - ((slotY - sBorder) * 2.0f / screenHeight) -
            uScale_slotBorder.y());
  ...
  for (int i = 0; i < m_slotCount; ++i) {
    ...
    if (tex) {
      ...
      drawTexturedQuad(gl, tex, iconX, iconY, iconW, iconH, screenWidth,
                       screenHeight, 1.0f);
    }
  }
  ...
  for (int i = 0; i < m_slotCount; ++i) {
    ...
    drawCooldownPieOverlay(gl, slotX, slotY, slotPx, slotPx, t, screenWidth,
                           screenHeight);
  }
  m_cooldownPieProgram->release();
}

void GLWeaponBar::drawTexturedQuad(QOpenGLFunctions_3_3_Core *gl,
                                   GLuint texture, float x, float y, float w,
                                   float h, float screenW, float screenH,
                                   float alpha) {

  QVector2D uScale(w * 2.0f / screenW, h * 2.0f / screenH);
  QVector2D uOffset((x * 2.0f / screenW) - 1.0f,
                    1.0f - (y * 2.0f / screenH) - uScale.y());
...
void GLWeaponBar::drawCooldownPieOverlay(QOpenGLFunctions_3_3_Core *gl, float x,
  ...
  m_cooldownPieProgram->setUniformValue(
      "uColor", QVector4D(0.3, 0.3, 0.3, 0.6)); // semi-transparent grey
