...
void Panel::setColor(const QVector4D &color) { m_color = color; }
...
void Panel::render(QOpenGLFunctions_3_3_Core *gl, float screenWidth,
                   float screenHeight) {
  if (!m_initialized)
    initialize(gl);
  ...
  QVector2D uScale(panelWidth * 2.0f / screenWidth,
                   panelHeight * 2.0f / screenHeight);
  QVector2D uOffset((panelX * 2.0f / screenWidth) - 1.0f,
