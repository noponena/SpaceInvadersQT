/*
        ...
        PURPOSE : Use OpenAL to render pre-mixed audio data from a S/W mixer.
   Optionally allow pre-mixed 'wet' buffers to feed hardware reverb effects (up
   to 4).

        BENEFITS : Compared to WASAPI, a native OpenAL device will communicate
        directly with audio drivers to by-pass the entire Vista audio system
   (Local APOs, mixer, Global APOs, etc ...).  This approach is more efficient
   and allows for much lower latency due to improved playback position
   monitoring. The hardware EAX reverb effects are efficient and very high
   quality.
*/
...
void MixData(unsigned long ulNumSamples, unsigned long ulOutputChannels,
             void *pOutput, unsigned long ulNumEffects, void **pEffectOutput);

// Some variables to control the data that is mixed into the buffers feeding
// OpenAL
...
void main() {
  ...
  if (InitializeOpenAL(NULL, 8, 2, 1024, 48000)) {
    ...
    if (g_pWaveLoader && pOutput && !bOutOfMemory) {
      // Load in a wave file
      if (SUCCEEDED(g_pWaveLoader->OpenWaveFile("..\\..\\Media\\sevenptone.wav",
                                                &g_WaveID))) {
        // Configure the hardware reverbs
        EFXEAXREVERBPROPERTIES EFXEAXReverb[2] = {EFX_EAX_REVERB_HANGAR,
                                                  EFX_EAX_REVERB_BATHROOM};
        ...
        do {
          ...
          if (_kbhit()) {
            ...
            printf("Effect Send 1 %.3f, Effect Send 2 %.3f\r",
                   g_flEffectSends[0], g_flEffectSends[1]);
          }

          // Periodically request the number of 'processed' Buffers (i.e. number
          // of blocks of requested number of samples (1024 in this example))
          ...
          while (ulBuffersProcessed) {
            MixData(ulNumSamples, ulOutputChannels, pOutput, ulNumEffects,
                    &pEffectOutput[0]);
...
//	pOutput				: Destination address for dry-mix
//(expects 'ulNumSamples' 16bit Samples for 'ulOutputChannels' Channels
//	ulNumEffects		: Number of effect outputs (wet-mixes)
//	pEffectOutput		: Array of desintation address for the wet-mixes
//(expect 'ulNumSamples' 16bit Samples for 1 output channel (i.e. mono))
void MixData(unsigned long ulNumSamples, unsigned long ulOutputChannels,
             void *pOutput, unsigned long ulNumEffects, void **pEffectOutput) {
  ...
  while (ulRequestedBytes) {
    if (g_bPlayTestWaveFile) {
      // Data for Dry Buffer
      g_pWaveLoader->ReadWaveData(g_WaveID, (char *)pOutput + ulBytesWritten,
  ...
  for (unsigned long ulEffectBuffers = 0; ulEffectBuffers < ulNumEffects;
       ulEffectBuffers++) {
    if (g_bPlayTestWaveFile && (g_flEffectSends[ulEffectBuffers] != 0.0f)) {
      for (ulSample = 0; ulSample < ulNumSamples; ulSample++) {
        ...
        for (ulChannel = 0; ulChannel < ulOutputChannels; ulChannel++)
          flMixSample +=
              ((short *)(pOutput))[(ulSample * ulOutputChannels) + ulChannel];
        ...
        ((short *)(pEffectOutput[ulEffectBuffers]))[ulSample] =
            (short)(flMixSample * g_flEffectSends[ulEffectBuffers]);
      }
    } else {
