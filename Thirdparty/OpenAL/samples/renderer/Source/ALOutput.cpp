...
bool InitializeOpenAL(char *szALDevice, unsigned long ulNumOutputChannels,
                      unsigned long ulNumEffects, unsigned long ulNumSamples,
  ...
  if (g_pDevice) {
    ...
    if (g_pContext) {
      ...
      if (alcGetError(g_pDevice) == ALC_NO_ERROR) {
        ...
        g_pData =
            (ALchar *)malloc(g_uiNumSamplesPerBuffer * g_uiOutputChannels * 2);
        if (g_pData) {
          ...
          if (alGetError() == AL_NO_ERROR) {
            ...
            OutputDebugMessage(
                "ALOutput : Failed to generate Source and / or Buffers\n");
        } else
          OutputDebugMessage("ALOutput : Out of memory\n");
      } else
        OutputDebugMessage(
            "ALOutput : Failed to make OpenAL context current\n");
    } else
...
unsigned long GetNumOutputChannels() { return g_uiOutputChannels; }

unsigned long GetNumEffects() { return g_uiNumEffectSlots; }

unsigned long GetNumSamplesPerBuffer() { return g_uiNumSamplesPerBuffer; }
...
bool Start() {
  ...
  if (g_bALInit) {
    ...
    for (uiLoop = 0; uiLoop < NUM_STREAMING_BUFFERS; uiLoop++) {
      alBufferData(g_uiBuffers[uiLoop], g_eFormat, g_pData,
                   g_uiNumSamplesPerBuffer * 2 * g_uiOutputChannels,
                   g_uiFrequency);
    ...
    for (uiLoop = 1; uiLoop < (g_uiNumEffectSlots + 1); uiLoop++) {
      for (uiInnerLoop = 0; uiInnerLoop < NUM_STREAMING_BUFFERS;
           uiInnerLoop++) {
        alBufferData(
            g_uiBuffers[(uiLoop * NUM_STREAMING_BUFFERS) + uiInnerLoop],
            AL_FORMAT_MONO16, g_pData, g_uiNumSamplesPerBuffer * 2,
            g_uiFrequency);
        alSourceQueueBuffers(
            g_uiSources[uiLoop], 1,
            &g_uiBuffers[(uiLoop * NUM_STREAMING_BUFFERS) + uiInnerLoop]);
      }
    }
...
unsigned long GetBuffersProcessed() {
  ...
  if (iState != AL_PLAYING) {
    ...
    OutputDebugMessage("ALOutput : One or more Sources were starved, stopping "
                       "all Sources (%d Buffers Processed)\n",
                       uiMinProcessed);
  }
  ...
  for (uiLoop = 0; uiLoop < (g_uiNumEffectSlots + 1); uiLoop++) {
    ...
    if (alGetError() == AL_NO_ERROR) {
      if (uiLoop == 0) // Dry Mix
      {
        ...
        alBufferData(uiBufferID, g_eFormat, pData,
                     g_uiNumSamplesPerBuffer * 2 * g_uiOutputChannels,
                     g_uiFrequency);
      } else {
        pData =
            pEffectOutputs[uiLoop - 1] ? pEffectOutputs[uiLoop - 1] : g_pData;
        alBufferData(uiBufferID, AL_FORMAT_MONO16, pData,
                     g_uiNumSamplesPerBuffer * 2, g_uiFrequency);
      }
...
void InitEFX(unsigned long ulNumEffects) {
  ...
  if (alGenAuxiliaryEffectSlots && alDeleteAuxiliaryEffectSlots &&
      alIsAuxiliaryEffectSlot && alAuxiliaryEffectSloti && alGenEffects &&
      alDeleteEffects && alIsEffect && alEffecti && alEffectf && alEffectfv &&
    ...
    if (g_uiNumEffectSlots) {
      // Generate a Filter to be used to mute the dry-path on the wet-mix
      // Sources
      alGetError();
      ...
      if (alGetError() == AL_NO_ERROR) {
        ...
        for (uiLoop = 0; uiLoop < g_uiNumEffectSlots; uiLoop++) {
          ...
          alSource3i(g_uiSources[uiLoop + 1], AL_AUXILIARY_SEND_FILTER,
                     g_uiEffectSlots[uiLoop], 0, AL_FILTER_NULL);
          // Generate Buffers for streaming
          alGenBuffers(NUM_STREAMING_BUFFERS,
                       &g_uiBuffers[(uiLoop + 1) * NUM_STREAMING_BUFFERS]);
          if (alGetError() != AL_NO_ERROR)
            break;
        ...
        OutputDebugMessage(
            "ALOutput : EFX Support not found (failed to create filter)\n");
        ShutdownEFX();
      }
    } else {
      OutputDebugMessage(
          "ALOutput : EFX Support not found (failed to create objects)\n");
      ShutdownEFX();
    }
  } else
    OutputDebugMessage(
        "ALOutput : EFX Support not found (missing function pointers)\n");
}
...
void ShutdownEFX() {
  ...
  for (uiLoop = 1; uiLoop < (g_uiNumEffectSlots + 1); uiLoop++) {
    ...
    for (uiInnerLoop = 0; uiInnerLoop < NUM_STREAMING_BUFFERS; uiInnerLoop++) {
      if (alIsBuffer(
              g_uiBuffers[(uiLoop * NUM_STREAMING_BUFFERS) + uiInnerLoop])) {
        alDeleteBuffers(
            1, &g_uiBuffers[(uiLoop * NUM_STREAMING_BUFFERS) + uiInnerLoop]);
  ...
  if (alIsAuxiliaryEffectSlot && alAuxiliaryEffectSloti &&
      alDeleteAuxiliaryEffectSlots && alIsEffect && alEffecti &&
      alDeleteEffects) {
    for (uiLoop = 0; uiLoop < g_uiNumEffectSlots; uiLoop++) {
      if (alIsAuxiliaryEffectSlot(g_uiEffectSlots[uiLoop])) {
        // Unload Reverb Effect
        alAuxiliaryEffectSloti(g_uiEffectSlots[uiLoop], AL_EFFECTSLOT_EFFECT,
                               AL_EFFECT_NULL);
...
bool SetEFXEAXReverbProperties(EFXEAXREVERBPROPERTIES *pEFXEAXReverb,
  ...
  if ((pEFXEAXReverb) && (ulEffectNumber < g_uiNumEffectSlots)) {
    ...
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_DENSITY,
              pEFXEAXReverb->flDensity);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_DIFFUSION,
              pEFXEAXReverb->flDiffusion);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_GAIN,
              pEFXEAXReverb->flGain);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_GAINHF,
              pEFXEAXReverb->flGainHF);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_GAINLF,
              pEFXEAXReverb->flGainLF);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_DECAY_TIME,
              pEFXEAXReverb->flDecayTime);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_DECAY_HFRATIO,
              pEFXEAXReverb->flDecayHFRatio);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_DECAY_LFRATIO,
              pEFXEAXReverb->flDecayLFRatio);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_REFLECTIONS_GAIN,
              pEFXEAXReverb->flReflectionsGain);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_REFLECTIONS_DELAY,
              pEFXEAXReverb->flReflectionsDelay);
    alEffectfv(g_uiEffects[ulEffectNumber], AL_EAXREVERB_REFLECTIONS_PAN,
               pEFXEAXReverb->flReflectionsPan);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_LATE_REVERB_GAIN,
              pEFXEAXReverb->flLateReverbGain);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_LATE_REVERB_DELAY,
              pEFXEAXReverb->flLateReverbDelay);
    alEffectfv(g_uiEffects[ulEffectNumber], AL_EAXREVERB_LATE_REVERB_PAN,
               pEFXEAXReverb->flLateReverbPan);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_ECHO_TIME,
              pEFXEAXReverb->flEchoTime);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_ECHO_DEPTH,
              pEFXEAXReverb->flEchoDepth);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_MODULATION_TIME,
              pEFXEAXReverb->flModulationTime);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_MODULATION_DEPTH,
              pEFXEAXReverb->flModulationDepth);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_AIR_ABSORPTION_GAINHF,
              pEFXEAXReverb->flAirAbsorptionGainHF);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_HFREFERENCE,
              pEFXEAXReverb->flHFReference);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_LFREFERENCE,
              pEFXEAXReverb->flLFReference);
    alEffectf(g_uiEffects[ulEffectNumber], AL_EAXREVERB_ROOM_ROLLOFF_FACTOR,
              pEFXEAXReverb->flRoomRolloffFactor);
    alEffecti(g_uiEffects[ulEffectNumber], AL_EAXREVERB_DECAY_HFLIMIT,
              pEFXEAXReverb->iDecayHFLimit);

    // Apply to Effect Slot
    alAuxiliaryEffectSloti(g_uiEffectSlots[ulEffectNumber],
                           AL_EFFECTSLOT_EFFECT, g_uiEffects[ulEffectNumber]);
