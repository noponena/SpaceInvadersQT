...
int main() {
  ...
  // Automatic Mode is the default, so there is no need to do any XRAM calls
  // before loading the data into the Buffer.  If there is available XRAM the
  // audio sample will be uploaded to XRAM, otherwise it will stay in Host RAM.
  ...
  // Load Wave file into Buffer using HARDWARE XRAM Mode (The
  // ALFWLoadWaveToBuffer function will set the Buffer Mode BEFORE making the
  // alBufferData call)
  if (ALFWLoadWaveToBuffer(ALFWaddMediaPath("wave2.wav"), uiBuffers[1],
                           bXRAM ? eXRAMHardware : 0)) {
    // Optional Query to find out the location of the Buffer
    if (bXRAM) {
  ...
  // Load Wave file into Buffer using ACCESSIBLE XRAM Mode (The
  // ALFWLoadWaveToBuffer function will set the Buffer Mode BEFORE making the
  // alBufferData call)

  // Use Accessible Mode for streaming buffers as it is more efficient to keep
  // the data on the host because the data will be constantly replaced when
  // servicing the Source's Queue of Buffers
  if (ALFWLoadWaveToBuffer(ALFWaddMediaPath("wave3.wav"), uiBuffers[2],
                           bXRAM ? eXRAMAccessible : 0)) {
    // Optional Query to find out the location of the Buffer
    if (bXRAM) {
