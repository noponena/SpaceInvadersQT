...
namespace GameObjects {namespace Ships {class Ship;}}
...
public:
    ...
    void shoot()
    {
        if (this->canShoot())
        {
            std::shared_ptr<GameObjects::Projectiles::Projectile> projectile = this->createProjectile();
    ...
    virtual std::shared_ptr<GameObjects::Projectiles::Projectile> createProjectile() = 0;
...
signals:
    void projectileShot(const std::shared_ptr<GameObjects::Projectiles::Projectile>& projectile);
};
