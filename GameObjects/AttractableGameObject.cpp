...
void AttractableGameObject::updateMovement(const UpdateContext &context)
{
    ...
    std::vector<std::pair<QPointF, Magnetism>> data = getMagneticObjectData(context);
...
void AttractableGameObject::applyMovement(float deltaTimeInSeconds, const QPointF &velocity)
{
...
void AttractableGameObject::handleMagneticMovement(const QPointF &totalMagneticForce, float deltaTimeInSeconds)
{
    QPointF normalizedForce = QVector2D(totalMagneticForce).normalized().toPointF();
...
const std::vector<std::pair<QPointF, Magnetism> > AttractableGameObject::getMagneticObjectData(const UpdateContext &context)
{
    ...
    for (const auto& objectPair : context.magneticGameObjects) {
        ...
        if (object->magnetism().enabled) {
            for (const auto& targetType : m_magneticTargets) {
                if (object->objectTypes().find(targetType) != object->objectTypes().end()) {
                    if (m_objectTypes.find(ObjectType::COLLECTABLE) != m_objectTypes.end()) {
