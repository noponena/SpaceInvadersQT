...
struct GameObjectBlueprint {
    ...
    // Movement
    std::optional<Game::Movement::MovementStrategy> movement;   // Or just an enum for the movement type and params

    // Child/variant specific fields
    std::optional<float> bossAuraRadius;
    std::optional<std::string> weaponBlueprintName;  // For enemies/ships with weapons

    std::unique_ptr<GameObjects::GameObject> instantiate(const Config::GameContext& ctx) const {
        ...
        // Child/variant-specific fields
        if (bossAuraRadius) {
            qDebug() << "[GameObjectBlueprint] ...bossAuraRadius present, value =" << *bossAuraRadius;
            // Example: Cast to BossEnemy and set bossAuraRadius
        }
        if (weaponBlueprintName) {
            qDebug() << "[GameObjectBlueprint] ...weaponBlueprintName present, value =" << QString::fromStdString(*weaponBlueprintName);
            // (You may want to spawn or look up the weapon by blueprint name)
        }

        // Finally, build the object (returns base class; cast if needed for child types)
