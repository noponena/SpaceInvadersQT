...
public:
     ...
     */
    static GameObjects::GameObjectBlueprint loadFromFile(const std::filesystem::path &path);

    /**
     * @brief Loads multiple GameObjectBlueprints from a YAML file containing a list of objects.
     * @param path Path to the YAML file.
     * @return Vector of GameObjectBlueprints.
     * @throws std::runtime_error if loading or parsing fails.
     */
    static std::vector<GameObjects::GameObjectBlueprint> loadMultipleFromFile(const std::string& path);
