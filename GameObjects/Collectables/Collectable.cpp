...
void GameObjects::Collectables::Collectable::update(const UpdateContext &context)
{
...
void Collectable::handleBlinking(float deltaTimeInSeconds)
{
    ...
    if (remainingLifePercent <= startBlinkingAt) {
        ...
        bool visible =
            shouldBlinkVisible(remainingLifePercent, m_blinkAccumulator);
        m_graphicsItem->setVisible(visible);
    }
}

bool Collectable::shouldBlinkVisible(float remainingLifePercent, float accumulator)
{
