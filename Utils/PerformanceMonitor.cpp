...
PerformanceMonitor::PerformanceMonitor(int logIntervalMs,
...
void PerformanceMonitor::initializeBenchmark(
...
float PerformanceMonitor::calculateBenchmarkScore() {
  if (m_stats.empty())
    return 0.0f;
...
void PerformanceMonitor::createFile() {
  openFile();
  if (m_outFile.is_open()) {
    writeHeader();
  }
...
void PerformanceMonitor::openFile() {
  m_outFile.open(m_filePath, std::ios::out | std::ios::app);
}

void PerformanceMonitor::closeFile() { m_outFile.close(); }
...
void PerformanceMonitor::writeHeader() {
...
void PerformanceMonitor::logPerformanceScore() {
