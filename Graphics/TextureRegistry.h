...
public:
    ...
    const TextureInfo& getOrCreateTexture(const QString& path) {
        ...
        if (img.isNull()) {
            ...
            qWarning() << "TextureRegistry: Could not load" << path << ", using placeholder.";
        ...
        m_gl->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, img.width(), img.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, img.bits());
...
private:
    ...
    void createPlaceholder() {
        ...
        if (m_placeholder.handle) return;
        ...
        m_gl->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 32, 32, 0, GL_RGBA, GL_UNSIGNED_BYTE, placeholder.bits());
        m_gl->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        m_gl->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        m_gl->glBindTexture(GL_TEXTURE_2D, 0);
