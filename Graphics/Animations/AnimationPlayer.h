...
public:
    AnimationPlayer()
        : m_currentFrame(0), m_timeAccumulatorMs(0), m_playing(false), m_currentAnimationFinished(false), m_loop(false) {}

    void update(int deltaMs) {
        if (!m_playing || m_animation.frameCount() == 0 || m_currentAnimationFinished) return;
    ...
    void reset() { m_currentFrame = 0; m_timeAccumulatorMs = 0; }
