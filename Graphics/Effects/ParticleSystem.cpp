...
void ParticleSystem::initializeGL(QOpenGLFunctions_3_3_Core *glFuncs) {
  ...
  glFuncs->glBufferData(GL_ARRAY_BUFFER, m_maxParticles * 6 * sizeof(float),
                        nullptr, GL_DYNAMIC_DRAW);

  glFuncs->glEnableVertexAttribArray(0);
  glFuncs->glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 6 * sizeof(float),
                                 (void *)0);
  glFuncs->glEnableVertexAttribArray(1);
  glFuncs->glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 6 * sizeof(float),
                                 (void *)(2 * sizeof(float)));
...
void ParticleSystem::uploadToGPU(QOpenGLFunctions_3_3_Core *glFuncs) {
  ...
  glFuncs->glBindBuffer(GL_ARRAY_BUFFER, m_vbo);
  glFuncs->glBufferSubData(GL_ARRAY_BUFFER, 0,
...
bool ParticleSystem::allParticlesDead() const { return m_allParticlesDead; }
...
void ParticleSystem::spawnParticles(int count, const QVector2D &pos,
                                    float lifeSpan, float maxSpeed,
