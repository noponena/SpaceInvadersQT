...
void GameState::initPlayerShip()
{
    ...
    std::shared_ptr<GameObjects::Ships::PlayerShip> playerShip = std::make_shared<GameObjects::Ships::PlayerShip>(100, m_playersShipStartSpeed, pos);
    playerShip->initialize();
    m_playerShip = playerShip;

    connect(playerShip.get(), &GameObjects::Ships::PlayerShip::projectileShot, this, &GameState::onProjectileShot);
...
void GameState::initEnemyShips()
{
    ...
    for (int j = 1; j <= rows; j++) {
        for (int i = 1; i <= cols; i++) {
            ...
            std::shared_ptr<GameObjects::Ships::EnemyShip> enemyShip = std::make_shared<GameObjects::Ships::EnemyShip>(10, 100, pos);
...
const std::shared_ptr<GameObjects::Ships::PlayerShip> &GameState::playerShip() const
{
    return m_playerShip;
}
}
