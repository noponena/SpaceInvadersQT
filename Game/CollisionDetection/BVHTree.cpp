...
std::shared_ptr<BVHNode> BVHTree::build(std::vector<GameObjects::GameObject *> &objects)
{
    if (objects.empty()) {
        return nullptr;
    }
    ...
    if (objects.size() == 1) {
        ...
        node->bbox = combineBoundingBoxes(node->leftChild->bbox, node->rightChild->bbox);
    }

    return node;
}

std::vector<GameObjects::GameObject*> BVHTree::query(std::shared_ptr<BVHNode> node, GameObjects::GameObject* queryObject) {
    QRectF queryBbox = queryObject->getBoundingBox();
    if (!node || !node->bbox.intersects(queryBbox)) return {};

    // If this is an internal node, check collisions between left and right children
    if (node->leftChild && node->rightChild) {
        ...
        leftResults.insert(leftResults.end(), rightResults.begin(), rightResults.end());
        return leftResults;
    }
    else {
        ...
        for (size_t i = 0; i < node->objects.size(); i++) {
            ...
            if (m_processedPairs.find(sortedPair) == m_processedPairs.end()) {
                ...
                if (queryObject->isCollidable() && node->objects[i]->isCollidable()) {
                    if (canCollide(queryObject->objectTypes(), node->objects[i]->objectTypes())) {
                        if (node->objects[i]->getBoundingBox().intersects(queryObject->getBoundingBox())) {
...
void BVHTree::clearProcessedPairs()
{
    m_processedPairs.clear();
}


QRectF BVHTree::calculateBoundingBox(const std::vector<GameObjects::GameObject*>& objects) {
    QRectF bbox;
...
std::pair<std::vector<GameObjects::GameObject*>, std::vector<GameObjects::GameObject*>> BVHTree::splitObjects(std::vector<GameObjects::GameObject*>& objects, bool splitX) {
    auto medianIt = objects.begin() + objects.size() / 2;
    std::nth_element(objects.begin(), medianIt, objects.end(), [splitX](const GameObjects::GameObject* a, const GameObjects::GameObject* b) {
        return splitX ? a->getCenterPosition().x() < b->getCenterPosition().x() : a->getCenterPosition().y() < b->getCenterPosition().y();
    });
...
bool BVHTree::shouldSplitX(const std::vector<GameObjects::GameObject*>& objects) {
