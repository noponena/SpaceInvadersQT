...
struct BVHNode {
    ...
    std::vector<GameObjects::GameObject*> objects; // Leaf nodes hold game objects
...
public:
    ...
    BVHTree();
    ~BVHTree();
    std::shared_ptr<BVHNode> build(std::vector<GameObjects::GameObject *> &objects);
    std::vector<GameObjects::GameObject *> query(std::shared_ptr<BVHNode> node, GameObjects::GameObject *queryObject);
    void clearProcessedPairs();

private:
    inline QRectF calculateBoundingBox(const std::vector<GameObjects::GameObject *> &objects);
    inline std::pair<std::vector<GameObjects::GameObject *>, std::vector<GameObjects::GameObject *> > splitObjects(std::vector<GameObjects::GameObject *> &objects, bool splitX);
    ...
    inline bool shouldSplitX(const std::vector<GameObjects::GameObject *> &objects);
};
