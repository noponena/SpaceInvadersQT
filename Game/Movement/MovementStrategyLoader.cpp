...
MovementStrategy MovementStrategyLoader::loadFromFile(const std::filesystem::path& path) {
    qDebug() << "[MovementStrategyLoader] Loading movement strategy from:" << QString::fromStdString(path.string());
...
MovementStrategy MovementStrategyLoader::loadFromYaml(const YAML::Node& node) {
    ...
    qDebug() << "[MovementStrategyLoader] Parsing movement strategy from YAML node...";
...
void MovementStrategyLoader::loadAxis(const YAML::Node& axisNode, std::vector<AxisMovementStrategy>& out) {
    int stepIdx = 0;
    for (const auto& stepNode : axisNode) {
        qDebug() << "[MovementStrategyLoader]   Parsing axis movement step #" << stepIdx++;
...
AxisMovementStrategy MovementStrategyLoader::parseStep(const YAML::Node& node) {
    ...
    qDebug() << "[MovementStrategyLoader]     Parsing axis step of type:" << QString::fromStdString(type);

    if (type == "Linear") {
        float speed = node["speed"].as<float>();
        int direction = node["direction"] ? node["direction"].as<int>() : 1;
        bool updateAnchor = node["updateAnchor"] ? node["updateAnchor"].as<bool>() : false;
        qDebug() << "[MovementStrategyLoader]     Linear: speed =" << speed << "direction =" << direction << "updateAnchor =" << updateAnchor;
        ...
        int direction = node["direction"] ? node["direction"].as<int>() : 1;
        bool updateAnchor = node["updateAnchor"] ? node["updateAnchor"].as<bool>() : false;
        qDebug() << "[MovementStrategyLoader]     Sinusoid: amplitude =" << amplitude << "frequency =" << frequency << "phase =" << phase << "direction =" << direction << "updateAnchor =" << updateAnchor;
        return SinusoidMovement(amplitude, frequency, phase, direction, updateAnchor);
        ...
        qDebug() << "[MovementStrategyLoader]     ERROR: Unknown axis movement type:" << QString::fromStdString(type);
        throw std::runtime_error("Unknown axis movement type: " + type);
    }
}

IntervalMovement MovementStrategyLoader::parseInterval(const YAML::Node& node, bool /*isXAxis*/) {
    ...
    if (node["intervals"]) {
        ...
        throw std::runtime_error("Interval movement missing 'interval' or 'intervals'");
    }

    std::vector<AxisMovementStrategy> subStrategies;
    if (!node["strategies"] || !node["strategies"].IsSequence()) {
        qDebug() << "[MovementStrategyLoader]       ERROR: Missing or invalid strategies";
        throw std::runtime_error("Interval movement must have a sequence of 'strategies'");
    }
    int stratIdx = 0;
    for (const auto& step : node["strategies"]) {
        qDebug() << "[MovementStrategyLoader]       Parsing sub-strategy #" << stratIdx++;
        subStrategies.push_back(parseStep(step));
    }
    qDebug() << "[MovementStrategyLoader]       IntervalMovement: strategies count =" << subStrategies.size() << "intervals count =" << intervals.size();
